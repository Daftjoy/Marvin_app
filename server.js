...
app.get('/request', async function (req, res) {
	...
	else
	{
		...
		axios.get("https://api.intra.42.fr/v2/campus/22", {
			headers: {
		...
		axios.get("https://api.intra.42.fr/v2/campus/22/events", {
			headers: {
			  'Authorization': 'Bearer ' + token
			}
		  }).then(function (response) {
			res.render(path.join(__dirname + '/app.ejs'), {events: response.data, campus: this.campus, req_ret: ''});
		  })
		  .catch(function (error) { 
			  	res.render(path.join(__dirname + '/app.ejs'), {events: 'Bad request.', campus: 'Bad request.', req_ret: ''});
		  });
	}
});
